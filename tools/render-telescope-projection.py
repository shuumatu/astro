...
for obj in data['mechanics']:
    if obj['part']=='internalDisk': continue  # Same explicit teaching visibility as scene.ts.
    faces = project(obj['positions']).reshape(-1,3,2)
    for face in faces:
        draw.polygon([tuple(p) for p in face],fill='#243746',outline='#304858')
...
draw.text((60,100),'Teaching view: internal perforated disk hidden. Source mechanical geometry is not an optical prescription.',fill='#e4b59d')
