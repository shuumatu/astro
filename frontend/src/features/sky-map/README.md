# Sky map worker

This module owns browser-side loading and calculation for the naked-eye catalog and
versioned sky-content packs. The main thread communicates only through
`SkyMapWorkerClient`; catalog parsing, culture selection, and all coordinate transforms
stay inside the module worker.

## Coordinate model

- Catalog positions and proper motions are ICRS. `pmRaMasPerYear` is
  `mu_alpha*cos(declination)` and is applied as a tangent vector, without dividing by
  `cos(declination)` a second time.
- `astronomy-engine` supplies the J2000-equatorial to local-horizon rotation, including
  precession, nutation, Earth rotation, and optional normal atmospheric refraction.
- The Sun, Moon, and seven planets use Astronomy Engine topocentric equatorial positions,
  illumination, distance, and the same optional atmospheric refraction setting.
- Culture figures reference physical stars by `HIP:{id}`. Figure vertices receive the
  same proper-motion propagation as rendered stars; a missing physical HIP record breaks
  the path instead of connecting unrelated vertices.
- Western IAU region boundaries are fixed ICRS `MultiPolygon` rings and do not receive
  proper motion. Chinese traditional content intentionally has no invented boundaries.
- The first version ignores stellar radial velocity, perspective acceleration, and annual
  parallax. These are below the visual precision needed for the naked-eye teaching chart,
  but the calculation date is limited to Julian years 1800 through 2200 to prevent misuse
  of the linear motion model.

## Rendering model

- The main renderer is a high-DPI Canvas 2D surface for the altitude grid, culture region
  boundaries, culture figures, featured patterns, thousands of visible stars, and Solar
  System body symbols, in that layer order.
- A small SVG overlay contains cardinal directions, localized culture and star labels,
  hover or selection markers, and localized Solar System labels. Star labels are filtered
  by brightness, data priority, zoom level, and collision checks. Form controls and
  numeric readouts remain regular DOM elements.
- Horizontal coordinates use an azimuthal-equidistant projection: zenith is at the center,
  the horizon is the rim, and north is at the top. Culture and region segments are clipped
  at the horizon before projection.
- Canvas backing dimensions follow the rendered size and cap device pixel ratio at 2 to
  preserve sharp output without excessive memory use.
- Zooming uses a pointer-anchored view transform while panning is constrained by the
  projected horizon. Star hit testing uses the same transformed screen coordinates.
- Target search accepts HIP identifiers, stable Solar System body IDs, and localized body
  names. Results share the same selection, focus, hit-testing, and readout flow; objects
  outside the current horizon or magnitude limit are reported as not visible.

## Integrity model

Browsers automatically decode gzip or Brotli `Content-Encoding` before exposing response
bytes to JavaScript. The backend therefore verifies the encoded `sha256` and
`contentLength`, while the worker verifies `decodedSha256` and `decodedContentLength`
before parsing JSON.

The physical catalog, sky-content manifest, global name index, and featured-pattern pack
load during worker initialization. Culture packs load on first use and are cached by
`cultureId`; the latest-calculation scheduler keeps the previous frame visible until a
new culture frame is ready.
