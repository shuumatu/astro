...
### The Sun, and why there is one angle control rather than two

The scene's light is a **real Sun position**: a point on the surface it stands directly over, given by
one selenographic longitude and the Moon's solar latitude. Its direction follows from that, so exactly
one hemisphere is lit, the terminator is a single great circle, and every place's solar elevation is a
consequence of where that place is rather than of where the viewer happens to be standing.

This replaced a lamp hung over the *camera*, positioned by a local azimuth and elevation. Such a lamp
follows the viewer around the surface, so the same place is lit from different directions depending on
which site you last flew to, and the "terminator" bends into shapes no phase of the Moon produces. It
...
- The **solar latitude** is not a control. It is the Moon's solar declination, taken from the real sky
  ...
  correct - a far-north or far-south site stands in a much lower Sun than an equatorial one.
- The dial **opens on a full Moon rather than on today's real phase.** Pinning it to today was tried and
  rejected: on the day it was written the real Moon was a 2 per cent crescent, so the near side opened
  almost black. A viewer who wants the real sky has one dial that reaches every phase; a viewer who
  wants to see terrain should not have to hunt for it. What is not modelled is libration, the Moon's
  few-degree wobble, so the terminator does not quite graze the poles as it would on a given date.

`full-bright` and `brightness` remain: the first removes the terminator so relief reads everywhere, and
the second scales every light together, which keeps the terminator's shape as the view brightens.
Neither is an angle, so neither competes with the one that is.
...
## Browsable surface panoramas

Selecting a landing site offers its surface panoramas, shown full screen and viewed from inside a cylinder.
They are generated by `tools/moon-textures/moon_panoramas.py`, which downloads each original, resizes it to
5120 px wide and encodes WebP; the geometry the viewer needs is written into the generated
`moonPanoramas.ts`.

| File | Source | What it is |
| --- | --- | --- |
| `panoramas/apollo-11-tranquility-base.webp` | NASA/JSC `jsc2007e045375` | Tranquility Base from west of the LM, 11 frames of magazine 40 |
| `panoramas/apollo-11-armstrong-crater.webp` | NASA/JSC `jsc2008e040725` | Armstrong's crater, 8 frames of magazine 40 |
| `panoramas/apollo-17-station-2.webp` | NASA/JSC `jsc2004e52777` | Station 2, EVA 2, 21 frames of magazine 138 |
| `panoramas/apollo-17-station-5.webp` | NASA/JSC `jsc2004e20304` | Station 5, EVA 2, 23 frames of magazine 145 |
| `panoramas/change-4-farside-lander.webp` | CNSA/CLEP via IAU `iau1901a` | The first far-side surface panorama, lander terrain camera |

NASA material is public domain in the United States with credit requested; the Chang'e 4 strip is CC BY 4.0.
Both credit lines travel with each panorama and are displayed under the image, so the attribution cannot drift
away from the photograph it belongs to.

### Why a cylinder, and how the framing is chosen

Three things had to be got right before these looked like photographs rather than smears, and all three are
worth knowing before changing any of the numbers.

**The surface is a cylinder, not a sphere.** These strips are cylindrical projections, and on a cylinder one
degree of azimuth is the same arc length as one degree of elevation, so a boulder keeps the shape the
photograph gave it. On a sphere the vertical scale is fixed by the geometry rather than by the photograph, and
the terrain came out stretched by roughly a factor of two. The calibration helper is
`docs/lunar-panorama-research/tools/flatten-view.py`, which reproduces the viewer's mapping on the CPU so a
change can be checked against the source pixels without a GPU.

**The cylinder's height is the perspective-correct `tan(vFov/2)`, and its width is the arc.** That pairing is
deliberate, and it is *not* isotropic: arc length per degree exceeds height per degree by `arc / vFov`, about
1.14 for the widest strips. It is nevertheless the correct pairing, because the camera opens at exactly `vFov`,
so this is the height that puts the top of the photograph at the top of the frame. Replacing it with an
isotropic surface was tried and is worse: these releases are anamorphic in the *other* direction, so making the
surface isotropic introduces a stretch rather than removing one. Apollo 11's strip, for instance, carries about
twice as many pixels per degree of elevation as per degree of azimuth, which is what the calibration factor
below is compensating for. Any future change here should be checked with `flatten-view.py` against the source
rather than reasoned about from isotropy.

**The viewer opens at exactly the strip's own vertical extent.** The photograph is then shown at the scale it
was measured at. Opening wider compresses it vertically: a 21 degree strip in a 34 degree camera loses a third
of its height and reads as horizontal streaking, which is what the first attempt looked like.

The elevation range is calibrated rather than derived. For a true cylindrical projection the sweep and the
elevation range are related by `sweep / aspect`, but these releases are not that, so the factor
(`VERTICAL_EXTENT_SCALE`, measured at 1.75 by flattening each strip at several candidate scales and comparing)
is applied and the result is what `verticalFovDeg` in `moonPanoramas.ts` records. It is the one fitted number
in this pipeline, and the tests assert only that it is applied - not what it is.

### What is uncertain, and what the interface says about it

- **`headingDeg`** is the azimuth the centre of the image faces. It is **0 for every entry**, so the panorama is
  placed at a neutral bearing. This was investigated rather than assumed: LPI's Apollo Image Atlas does print a
  `Camera Azimuth` per frame, but only on a small minority of frames - 1 of 17 sampled across the eight
  magazines the panoramas come from, and none at all among the frames actually stitched here - and the Apollo
  Lunar Surface Journal, which gives solar azimuths and bearings to named landmarks, publishes its own warning
  that such per-frame azimuths "may have errors of 3° or more". Nothing available is accurate enough to encode.
  Closing it would mean a photogrammetric solve against the LROC NAC mosaics, which is a research task rather
  than a lookup. **Because the viewer labels no compass direction, this costs the interface nothing visible** -
  an earlier version carried a disclaimer that north on screen was not north on the Moon, which described a
  direction marker that does not exist, and that wording has been removed.
- **`spanDeg`** is the azimuth the strip covers, and `spanSource` records how firmly that is known:
  `documented` (a source states it), `closes` (the two ends were measured to join) or `modelled` (from the
  frame count and the overlap the mission reports). **No strip here is marked `closes`.** Every one of them was
  checked by putting its two ends side by side, and in every case they show different terrain at different
  horizon heights - including the three that earlier notes in this repository claimed were joined. A strip
  whose ends do not meet therefore stops at the edge of the photograph instead of wrapping, and every entry
  carries the note that says so.
- **A strip's sweep cannot be verified from the file**, only made self-consistent, which is why the spans are
  `modelled` and why the viewer confines each one to its own arc rather than allowing unlimited turning. The
  source releases do not state a sweep. If a span is ever tightened - from NASA's frame lists, or from the
  traverse maps - only the manifest in `tools/moon-textures/moon_panoramas.py` needs to change.

See `docs/lunar-panorama-research/` for the source survey, the measurements, the wrap tests, and
the material that was investigated and rejected.

Apollo 12, 14, 15 and 16 panoramas are catalogued in the tool's manifest but marked `pending`: they need
terrain crops to fly into before they can be published, and the Moon Trek image service those crops come from
was unreachable when this was built. Removing the `pending` note for an entry once its site has crops is all
that is needed to ship it.
